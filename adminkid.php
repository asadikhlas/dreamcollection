		...
		<div class="flex-w flex-c-m m-tb-10">
            ...
            <?php
                    ...
                    if(isset($_GET['inputTxt'])){
                      ...
                      $q = "SELECT DISTINCT (p.postId), i.image ,p.postName, p.postType, p.postPrice, p.postDate, i.image from images i RIGHT JOIN postad p ON i.postId = p.postId WHERE p.postCategory = 'kid' AND  postName LIKE '%$input%' GROUP BY p.postId";
        			$res = $conn->query($q); 
                      ...
                      $q1 = "SELECT DISTINCT (p.postId), i.image ,p.postName, p.postType, p.postPrice, p.postDate, i.image from images i RIGHT JOIN postad p ON i.postId = p.postId WHERE p.postCategory = 'kid' AND postName LIKE '%$input%' GROUP BY p.postId LIMIT $pageStartingRow, $resultsPerPage;";
                      $res1 = $conn->query($q1);
				if($row >= 1){
            ...
            if($noOfPages > 1){ //pagination would show when total pages($noOfPages) are greater then 1... 
              ...
              if($currentPage > 1){ //if current page is greater then 1 then < (precious page indicator) would show.
                echo "<li class='page-item' style='background-color:#717fe0; border-color:#717fe0'>
				      <a class='page-link' style='color:#717fe0'; href='adminkid.php?page=$prevPage' aria-label='Previous'>
                ...
                for($page=$currentPage - 1; $page<= $currentPage + 1; $page++){//looping throgh page buttons.
                  if($currentPage == $page){//making current page active(current page)
                    //echo "current";
                      ...
                      echo "<li class='page-item'><a class='page-link' href='adminkid.php?page=$page'>$page</a></li>";
                    }  
                  }
                }
              ...
              if($noOfPages > 1 AND $currentPage < $noOfPages){
                ...
                echo "<a class='page-link' style='color:#717fe0;' href='adminkid.php?page=$nextPage' aria-label='Next'>
